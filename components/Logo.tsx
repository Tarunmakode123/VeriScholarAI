...
export const Logo: React.FC<LogoProps> = ({ className = '', size = 'md', showSubtitle = false }) => {
  const pxSize = size === 'sm' ? 24 : size === 'lg' ? 40 : 32;
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      <div 
        style={{ width: `${pxSize}px`, height: `${pxSize}px`, minWidth: `${pxSize}px`, minHeight: `${pxSize}px` }}
        className={`${iconSize} relative flex items-center justify-center rounded-lg bg-gradient-to-br from-brand-500 to-brand-600 text-white shadow-md shadow-brand-500/20 shrink-0 overflow-hidden`}
      >
        {/* Stylized V + Shield + Check mark logo */}
        <svg 
          viewBox="0 0 24 24" 
          fill="none" 
          style={{ width: `${pxSize * 0.7}px`, height: `${pxSize * 0.7}px` }}
